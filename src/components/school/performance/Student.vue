      ...
      <div class="result clear">
        ...
        <div style="margin: 10px;overflow: hidden">
          <div style="float: right;">
            <Page :total="this.$store.state.stuAwardList.length" :current="1" @on-change="changePage"></Page>
          </div>
        </div>
      </div>
  ...
  export default{
            ...
            render: (h, params) => {
              return h('div', [
                h('Button', {
                  ...
                  on: {
                    click: () => {
                      this.$router.push('/school/performance/student_detail/' + params.row.no)
                    }
                  }
                }, '查看详情')
              ])
    ...
    methods: {
      ...
      getCollegeData (index) {
        ...
        return this.$store.state.stuAwardList.slice((index - 1) * 10, index * 10)
      },
      changePage (index) {
